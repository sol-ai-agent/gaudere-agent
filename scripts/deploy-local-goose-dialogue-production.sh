...
authorization=${GAUDERE_LOCAL_GOOSE_DIALOGUE_DEPLOY_AUTHORIZATION:-}
expected_provider_total=${GAUDERE_EXPECTED_PROVIDER_TOTAL:-10}
expected_previous_image=${GAUDERE_EXPECTED_PREVIOUS_IMAGE:-321df97f8b03e12de1727ad13c5e9d05646fb807e52fbef9093fc61127b98595}
...
fail()
{
    printf 'gaudere Local Goose dialogue production deploy: FAIL: %s\n' "$*" >&2
    exit 1
}
...
[ "$dialogue_before" = "0" ] || fail "production already contains Local Goose dialogue Tasks"
...
candidate_tag="localhost/gaudere-agent:local-goose-dialogue-$agent_ref"
...
"$podman_command" run --rm --network none --read-only --read-only-tmpfs --security-opt=no-new-privileges --cap-drop=all --entrypoint /usr/local/bin/gaudere-control "$candidate_id" --help 2>&1 | grep -q 'local-message' || fail "candidate control client lacks local-message"
...
recover()
{
    ...
    printf 'gaudere Local Goose dialogue deploy: recovery starting\n' >&2
...
printf '=== START PRODUCTION WITH DIALOGUE CODE WIRED BUT IDLE ===\n'
...
[ "$dialogue_after" = "$dialogue_before" ] || fail "code-only deploy created a dialogue Task"
...
printf 'FEDORA_LOCAL_GOOSE_DIALOGUE_CODE_DEPLOY=PASS\n'
