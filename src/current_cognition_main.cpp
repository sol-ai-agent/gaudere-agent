...
void usage(const char* program)
{
    std::cout
        << "Usage: " << program
        << " --state PATH --task-id ID --check\n"
        << "       " << program
        << " --state PATH --task-id ID"
        << " --execute-after-explicit-current-cognition-go"
...
Options parse_options(const int argc, char* argv[])
{
    ...
    for (int index = 1; index < argc; ++index) {
        ...
        } else if (argument == "--task-id" && index + 1 < argc) {
            options.task_id = argv[++index];
        } else if (argument == "--check") {
            options.check = true;
        } else if (argument == "--execute-after-explicit-current-cognition-go") {
    ...
    if (options.task_id.empty() || options.task_id.size() > max_task_id_bytes)
        throw std::invalid_argument("--task-id ID must be 1..1024 bytes");
    if (options.check == options.execute) {
        throw std::invalid_argument(
            "choose exactly one of --check or --execute-after-explicit-current-cognition-go");
    }
    ...
    if (options.execute && options.model.empty()) {
        throw std::invalid_argument(
            "--execute-after-explicit-current-cognition-go requires --openai-model");
    }
...
{
    const auto captured =
        gaudere_agent::current_cognition_snapshot_captured_at_ms(task);
    ...
    const auto max_age = std::chrono::duration_cast<std::chrono::milliseconds>(
        gaudere_agent::current_cognition_max_snapshot_age).count();
...
int main(int argc, char* argv[])
{
    try {
        ...
        // Both modes require exclusive ownership so the exact Task inspected is the
        // one later executed. The caller must deliberately stop the Gaudere service.
        ...
        const auto task = task_store.find(options.task_id);
        if (!task)
            throw std::runtime_error("durable current cognition Task is missing");
        if (!gaudere_agent::valid_current_cognition_task(*task)) {
            throw std::runtime_error(
                "durable current cognition Task is non-canonical");
        }
        ...
        std::cout << "current_task_id=" << options.task_id << '\n'
        ...
        if (options.check) {
            std::cout << "mode=check\nprovider_effects=0\n";
            return !gaudere::work::is_terminal(task->status)
                    && fresh
        ...
        if (gaudere::work::is_terminal(task->status)) {
            std::cout << "provider_call_started=false\nreason=current_task_terminal\n";
            return 0;
        }
        if (!fresh) {
            std::cerr << "gaudere-current-cognition: frozen context is not fresh enough for a real provider call\n";
            return 3;
        }
        if (budget.next_new_consumption != gaudere::budget::ConsumeResult::accepted) {
            std::cerr << "gaudere-current-cognition: provider budget does not admit a new call\n";
            return 4;
        }
        ...
        gaudere_agent::CurrentCognitionHandler cognition(activation.handler());
        gaudere_agent::TaskExecutor executor(work_runtime, task_store);

        const auto execution = executor.execute(
            options.task_id, "current-cognition-one-shot", cognition);
        if (execution != gaudere_agent::ExecuteResult::completed) {
            std::cerr << "gaudere-current-cognition: execution did not complete\n";
            return 5;
        }
        ...
        if (!stored || !gaudere::work::is_terminal(stored->status)) {
            throw std::runtime_error(
                "current cognition Task is not terminal after execution");
        }
        ...
        std::cerr << "gaudere-current-cognition: " << error.what() << '\n';
        return 1;
    }
}
