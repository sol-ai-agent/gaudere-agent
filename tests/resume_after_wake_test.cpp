                               ...
                               const std::string& message)
{
    ...
    expect(!fixture.actions.find(provider_action_id(wake_id))
               && !fixture.actions.find_by_idempotency_key(
                    provider_action_id(wake_id)),
           message + ": no provider Action exists");
}

void test_disabled_is_inert_and_direct_store_is_read_only()
{
    ...
    // Resume authority reads durable wake evidence directly. The ExplicitWake
    // object is used only above to manufacture the fired fixture and is not a
    // dependency of ResumeAfterWake.
    ResumeAfterWake resume(fixture.tasks, fixture.wakes,
                           fixture.work_runtime, false);
...
void test_one_claim_repeated_reconciliation_and_unrelated_state()
{
    ...
    const auto canonical_input =
        accepted.task ? accepted.task->input : std::string{};
...
void test_reopen_before_and_after_claim()
{
...

    {
        ...
        ResumeAfterWake resume(reopened.tasks, reopened.wakes,
                               reopened.work_runtime, true);
...
int main()
{
    test_disabled_is_inert_and_direct_store_is_read_only();
